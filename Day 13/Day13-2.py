...
class Point:
    ...
    def __cmp__(self, other):
        y_cmp = self.y - other.y
        return y_cmp if y_cmp != 0 else self.x - other.x
...
intersection_decisions = ["left", "straight", "right"]


def turn_at_intersection(turn_index, current_direction):
    intersection_type = intersection_decisions[turn_index % 3]
...
carts = [Cart(Point(x, y), puzzle_input[y][x]) for x in range(width) for y in range(height) if puzzle_input[y][x] in cart_symbols]
...
def tick():
    ...
    for cart in carts:
        ...
        if has_cart(next_location):
            cart.crash()
            has_cart(next_location)[0].crash()
            continue
...
while True:
    tick()
    not_crashed = list(filter(lambda cart: not cart.crashed, carts))
    if len(not_crashed) <= 1:
        print(list(map(str, not_crashed)))
        break
